...
export class TreasureHuntManager {
    ...
    createTreasureFoundMessage(time: Date) {
        ...
        const text = `Congratulation!, you found the treasure at ${time.getMilliseconds()} past the ${time.getSeconds()} second of the ${time.getMinutes()} minute of the ${time.getHours()} hour. That is ${time.toUTCString()} in UTC. Send this to @aarohiworld on Instagram for a chance to win fabulous prizes.`
